...
계층 1·3의 아래 항목은 구현 전에 먼저 한 문장씩 고정해야 한다.
...
1. GDN-PCA와 GDN-MOMENT_ZS를 모두 주 비교로 두면 비율 7개씩 총 14개 검정이다.
   Bonferroni `0.05/7`은 두 비교를 별도 family로 볼 때만 맞다. 하나의 family인지 두
   family인지 사전 등록해야 한다.
...
GHL 25개가 같은 simulator family라 일반 모집단 표본이 아니라는 제한, 교차점을 관측한 일곱
비율 안의 이산 위치로만 보고한다는 원칙은 유지해도 된다.

## 회의 순서

1. raw·smoothed 가운데 어느 쪽을 본 결과로 쓸지 확정한다.
2. 각 모델의 공식 구현과 고정값을 모델별로 하나씩 봉인한 뒤 구현한다.
3. 지우가 VUS-PR·ℓ_max를, 주혜가 난이도·다중검정·δ를 봉인한다.
4. 서버 데이터 경로를 확인한 뒤 합성 dry-run을 한 번만 실행한다.
5. 위 항목이 모두 끝났을 때만 GHL·HAI 실데이터 학습을 연다.
