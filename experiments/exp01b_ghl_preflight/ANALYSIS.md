...
- 학습 전체의 IQR=0 채널 행: 150 / 475개. 서로 다른 값이 1개뿐인 완전 고정 행은 0개다. IQR=0 채널은 Relaxing.active, boundary.m_flow_in, dir_valve_act, inj_valve_act, inv_valve_act, out_valve_act이며, 희소하게 전환되는 제어 신호라서 삭제 대상으로 단정하지 않는다.
- 5% 구간의 IQR=0 채널 행: 279 / 475개. 서로 다른 값이 1개뿐인 완전 고정 행은 154개다. 해당 행은 C_level, C_temperature.T, Relaxing.active, dL_rand, dT_rand, dt_rand, inv_valve_act, limiter.y, limiter1.y, out_valve_act에서 나왔다. 상수열도 부동소수점 표준편차가 미세하게 0을 벗어나므로 완전 고정 판정에는 `unique_value_count=1`을 쓴다. 비율마다 정규화 통계를 다시 추정해야 한다는 근거다.
- 25개 × 5비율 × W 후보 5개 = 625조건 가운데 train·validation·정규화 표본이 1개라도 없는 조건: 0개. 정규화 표본 수는 D-34의 현행 점수 길이와 같은 `L-W`로 센다.
...
GHL 파일 로더와 배치 러너는 지금 작성해도 된다. 실제 입력 계약은 25개, 19채널, 마지막 열 `Label`, 파일명의 `tr_` 값을 경계로 쓰는 형태로 고정됐다. 가장 짧은 series 12도 5% 조건에서 W=155일 때 train 윈도 1,642개, validation 윈도 45개, 정규화 표본 1,842개가 남는다. 비율별 정규화 통계는 해당 비율의 학습 구간에서 다시 추정하고, 5%·10%에서 멈춘 채널을 임의로 삭제하지 않는다.
