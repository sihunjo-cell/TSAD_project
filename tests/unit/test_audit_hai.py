...
class TestBuildRatioFeasibilityRows(unittest.TestCase):
    def test_shortest_hai_session_checks_only_current_window(self):
        ...
        row = rows[0]
        self.assertEqual(row["available_length"], 12600)
        self.assertEqual(row["fit_length"], 10080)
        self.assertEqual(row["validation_length"], 2520)
        self.assertEqual(row["model_train_length"], 10080)
        self.assertEqual(row["train_window_count"], 10075)
        self.assertEqual(row["validation_window_count"], 2515)
        self.assertEqual(row["scaler_fit_observation_count"], 10080)
        self.assertEqual(row["validation_transform_observation_count"], 2520)
