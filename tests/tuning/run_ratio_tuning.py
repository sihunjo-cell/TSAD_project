...
from tests.tuning.build_ratio_tuning_budget import (
...
from tests.tuning.store_recommendation_evidence import (
...
from tests.tuning.compare_execution_runtimes import load_runtime_references, summarize_runtime_comparisons


def _paths(*, full_prefix=False):
    if full_prefix:
        return {
            "budget": tuning.SNAPSHOT_DIRECTORY / "budget.json",
            "result": tuning.DEFAULT_RESULT_DIRECTORY,
            "manifest": tuning.DEFAULT_SCORE_MANIFEST_PATH.with_name("dev18_full_prefix_v2_manifest.csv"),
            "checkpoint": tuning.DEFAULT_RESULT_DIRECTORY / "vus_checkpoints",
...
def _prepare_full_prefix_tuning(registry, registry_sha):
    ...
    from tests.tuning.build_dev18_feasibility import _load_approved_inventory
    ...
    for field, relative in (
        ("builder_sha256", "tests/tuning/build_ratio_tuning_budget.py"),
...
def main():
    ...
    with hold_tuning_lock(tuning.REPOSITORY_ROOT / ".runtime/ratio_tuning.lock"):
        ...
        if report.get("experiment_mode") == "full_prefix_v2" and report.get("status") == "ready_for_handoff":
            from tests.tuning.package_recommendation_handoff import package_recommendation_handoff
