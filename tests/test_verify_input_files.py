...
class TestVerifyInputFiles(unittest.TestCase):
    ...
    def test_rejects_file_changed_after_manifest_verification(self):
        with tempfile.TemporaryDirectory() as temporary_dir:
            root = Path(temporary_dir)
            input_path = root / "input.csv"
            input_path.write_bytes(b"abc")
            fingerprints = verify_input_files("TEST", root, self.write_manifest(root))

            input_path.write_bytes(b"abd")
            changed_mtime_ns = fingerprints[0]["mtime_ns"] + 1_000_000_000
            os.utime(input_path, ns=(changed_mtime_ns, changed_mtime_ns))

            with self.assertRaisesRegex(RuntimeError, "검증 뒤 바뀌었다"):
                verify_input_file_state(root, fingerprints)
