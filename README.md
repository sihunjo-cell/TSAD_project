...
심화세션 5조 시훈, 강혁, 지우, 주혜 프로젝트. 제조시계열 상에서 딥러닝 기반 이상탐지 모델은 데이터가 어느정도 쌓였을 때 의미있는 정도(연속적으로)로 경량통계 모델을 이기는가? (컨퍼런스 주제의 압축, 쉬운버전) 모델 코드, 실험 코드,실험 결과를 서로 다른 폴더에 둔다.
...
[Tier 2 EDA 보고서](docs/lead/tier2_parameter_eda_report.ipynb),
