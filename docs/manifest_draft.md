...
행 수는 헤더를 뺀 데이터 행만 센 값이다. 열 수는 CSV에 적힌 모든 열을 포함한다. 원본은 저장소 밖 공용 경로에 두었으며, 이 문서에는 경로와 검수 결과만 남긴다.

## GHL

- 원본 경로: `C:\Users\simon\time_series\shared_data\TSAD_project\TSB-AD-M`
- 공식 목록: TheDatumOrg/TSB-AD `Datasets/File_List/TSB-AD-M.csv:33-57`
...
- 원본 경로: `C:\Users\simon\time_series\shared_data\TSAD_project\HAI-23.05`
- 기준 버전: `icsdataset/hai` 커밋 `2a814cebc9a66b06c9e5cd545e2d72e65d383737`의 HAI 23.05
- 기술문서: `C:\Users\simon\time_series\shared_data\TSAD_project\references\hai_dataset_technical_details.pdf`, PDF 2쪽(문서 1쪽)과 PDF 31쪽(문서 30쪽)
- 획득 검수: GitHub에서 `git lfs pull --include="hai-23.05/*"`을 실행했으나 저장소 LFS 용량 초과로 객체 전송이 막혔다. 공식 README `README.md:5`가 안내하는 Kaggle 미러에서 같은 파일을 받고, `hai-23.05/*:1-3`의 LFS SHA-256과 바이트 크기를 10개 모두 대조했다.
...
HAI 23.05를 쓴다. HAIEnd 23.05는 같은 실험에서 수집한 225채널 DCS 내부 논리 데이터라 계획서의 HAI 59~86채널 범위와 다르다. HAI 23.05는 86채널이며 계획서가 잡은 대표세트와 일치한다. 공식 `README.md:24, 34, 58-95, 240-246`과 기술문서 PDF 2·31쪽이 버전, 채널 수, 파일별 시간 연속성을 뒷받침한다.
...
모델 입력에서 timestamp와 label을 빼면 `N=86`이다. D-17 규칙을 적용한 HAI GDN 이웃 수는 `k=max(5, ceil(0.25×86))=22`로 확정한다. 이 값은 모든 HAI 비율과 seed에 공통으로 쓴다.

### 실험 전 전처리 결정

GHL·HAI 모두 다운샘플 배율 1, 초기 절단 0포인트로 고정한다. timestamp와 label은 입력에서 뺀다. 입력 센서는 validation 분할 뒤 train 부분에만 `MinMaxScaler`를 fit하고 같은 scaler로 validation과 test를 변환한다. GHL은 현재 시계열의 train 부분마다 scaler 하나를 fit한다. HAI는 훈련 4세션 각각에 비율과 validation 분할을 적용한 뒤 네 train 부분으로 scaler 하나를 fit한다. 세션 경계에는 윈도를 만들지 않는다. 기계 판독값은 `configs/data_preprocessing.yaml`, HAI 훈련 데이터 근거와 10% 분석은 `experiments/exp01c_hai_preflight/ANALYSIS.md`에 있다.
