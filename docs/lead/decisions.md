...
## PCA 성분 거리의 가용 CPU 사용

분해만 병렬화한 코드에 남아 있던 직렬 성분 거리를 같은 프로세스의 스레드로 나눠 계산한다.
기존 1,024행 묶음·공식 cdist·가중합·모든 성분을 유지하며 할당 CPU와 남은 묶음 수에 맞춰
작업 수를 정한다. 분해 전 입력 배열·solver·OpenBLAS 초기화 경로는 바꾸지 않는다. 분해 완료와
거리 진행을 출력하고 선택 성분 수·실제 거리 작업 수·단계 시간을 남긴다.

과거 snapshot의 거리 계산은 1작업이었다. 새 snapshot에는 거리 요청 작업 수를 추가하며
비교용 실측쌍은 fit과 거리의 병렬 수를 모두 맞춘다. 같은 입력·설정의 기존 1스레드 실측 재사용과
과거 설정 간 시간비 추정은 그대로다. 실제 실행·중단 비용은 보존하고 근거 부재로 실행을 막지 않는다.
최대 CPU 사용이 선형 가속이나 정확한 1스레드 시간 복원을 보장하지는 않는다. 원격 회귀가 남았다.

## PCA 병렬 SVD의 초기화 충돌 복구

Lightning 합성 대조에서 OpenBLAS를 1스레드로 시작한 뒤 16개로 늘릴 때만 SIGSEGV가 재현됐다.
진입점에서 NumPy·SciPy BLAS를 affinity CPU 수로 초기화하고 평상시에는 1스레드로 낮춘다.
PCA fit에서만 기존 CPU quota를 반영한 가용 수를 쓰고 종료 후 복원한다. 라이브러리·PCA
solver·후보·점수식·기존 비교 시간은 바꾸지 않는다. 실제 큰 입력의 성공과 가속률은 원격 확인 전이다.

충돌 복구를 위해 예산 `b2f61f74691c6`의 PCA series 13·`cb3ca230f385a`·r100·seed 0에
attempt 3을 한 번 허용한다. 예산의 3회 한도와 기존 이력을 고쳐 쓰지 않고 새 실행 이력에
복구 사유와 확장 한도를 기록한다. 다른 trial이나 반복 복구에는 적용하지 않으며 실패·중단
시도의 비용 기록도 보존한다.

## 현재 서버의 가용 CPU 사용

사용자 요청으로 PCA fit과 VUS-PR 채점의 고정 상한 8을 없앤다. 기존 의존성의 CPU 감지로
affinity와 컨테이너 할당을 반영한다. PCA는 프로세스 시작에 정한 스레드 수를 fit과 snapshot에
함께 쓰며 종료·예외 때 이전 BLAS 설정으로 복원한다. VUS-PR은 같은 CPU 감지와 가용 RAM·
남은 작업 수에 맞춰 프로세스를 배분하고 내부 스레드는 1개로 유지한다. CPU 정보는 실행별로
기록하며 과거 snapshot과 모델의 환경 봉인을 고쳐 쓰지 않는다.

비교 시간은 기존 1스레드 실측을 먼저 연결한다. 새 실행이 16스레드라면 1/16 관측쌍을 쓰며
1/8의 시간비를 가져오지 않는다. 근거가 없을 때 기존 PCA 설정 간 시간비를 쓰는 원칙도 유지한다.
VUS-PR 병렬 수는 채점 소요시간만 바꾸며 모델별 학습·추론시간에 합산하지 않는다.
자원 변경으로 commit이 달라져도 채점 입력·라벨·평가기·예산 신원이 같고 승인된 소스끼리
호환되면 기존 체크포인트를 재사용한다. 원본 체크포인트를 덮어쓰거나 무관한 소스 변경을 허용하지 않는다.

## 기존 PCA 기록을 이용한 비교 시간

PCA 병렬화만 적용하고 기존 모델과 비교할 시간의 연결을 빠뜨린 수정을 보완한다.
`comparison_runtime_seconds`는 기존 모델과 1스레드 PCA에서는 원래 실측값이다. 새 PCA에는
같은 입력 SHA·평가 범위·설정·seed·실행 방식·환경의 1스레드 실측을 우선 연결한다. 과거
스레드 수가 없으면 확인한 PCA·진입점 소스의 1스레드 정책을 근거로 표시한다. 과거 CPU 모델
기록이 없으므로 장비 연속성은 사용자 설명에 근거하며 다른 장비의 동일성을 입증한 값은 아니다.

같은 조건의 실측이 없으면 동일 설정의 1/8스레드 실측쌍에서 입력별 시간비를 구한다. 쌍도
없으면 대상 입력의 다른 PCA 설정 실측에, 다른 입력들에서 관측한 대상 설정/기준 설정
시간비를 곱한다. 반복 기록은 입력·설정·스레드별 중앙값으로 줄이고 여러 기준 설정의 예측을
입력별 중앙값으로 합친 뒤 입력 간 중앙값을 쓴다. 이는 기존 측정에 근거한 비용 비교 보완이며
공식 PCA 절차나 보장된 1스레드 시간 복원이 아니다. 입력 크기·채널·solver 차이에 따른 전이
오차가 있으므로 입력별 예측·근거 파일·SHA와 예측 최솟값·최댓값을 보존한다. 신뢰구간으로
부르지 않는다. 타 모델의 시간비를 PCA 계수로 쓰지 않고 q·head 참조도 실행 표본으로 늘리지 않는다.

실제 `runtime_seconds`는 기존 timing 합으로 유지한다. 비교용 필드는 지원 근거의 executions와
비용 이력에 연결하고 status로 실측·추정·미확정을 구분한다. 근거가 없으면 실행을 막지 않으며
CPU core-seconds로 실제 시간을 채우지 않는다. 이번 계산은 새 모델 실행 없이 같은 예산의
완료 이력만 사용한다. 실패·중단 시도는 실제 개발비에 남기고 성공 실행시간의 추정 표본에서 뺀다.

## PCA 8스레드 실행과 1코어 환산 계산량

사용자 요청에 따라 PCA fit의 BLAS 스레드만 8개로 늘린다. fit 종료·예외 때 이전 설정으로
복원하며 후보·전처리·점수식과 나머지 모델의 스레드 설정은 유지한다. 같은 L4·CPU 8개 환경에서
기존 완료 결과를 재사용하고 미완료 PCA trial은 처음부터 실행한다. 이번 변경의 검토한 파일
내용만 재개 호환으로 인정한다. 기존 실행의 commit과 측정값은 고쳐 쓰지 않는다.

각 PCA 실행의 실제 경과시간과 모든 프로세스 스레드의 CPU 시간 합계를 따로 기록한다.
CPU 시간 합계를 1코어 환산 계산량으로 쓰며 병렬 처리의 추가 연산·대기 중 CPU 사용도 포함한다.
이는 실제 1스레드 실행시간이나 공급자 청구액이 아니다. `경과시간 × 8`로 채우거나 기존
`runtime_seconds`를 환산값으로 덮어쓰지 않는다. 다른 모델의 경과시간과 비교해 1코어 실측
비용이라고 보고하려면 같은 조건의 1스레드 측정이 필요하다. 이 측정을 튜닝 재개 조건으로
추가하지 않으며, 과거 CPU 시간 미측정도 재개를 막지 않는다.
