...
2026-09-09 첨부된 Lightning 자원 보고서에서 TSPulse 세 설정의 실제 CUDA OOM을 확인했다.
series 13은 248채널이며 나머지 모델 점검은 통과했다. 공식 경로가 추론 배치 제한 32를 128로
덮어쓰던 부분을 제거하고 registry·실행 metadata를 32로 맞췄다. 실패 보고서는 신원 검사보다
먼저 모델·원래 오류·GPU/RAM 사용률을 표시한다. 후보·창·head·공식 점수식과 80% 기준은 유지한다.
현재 게이트는 수정·정적 검토, 원격 확인 전이다. 로컬 Python·테스트·모델은 실행하지 않았다.
다음은 관련 호출 인자·오류 보고 회귀와 기존 명령 재실행이다. 배치가 registry 신원을 바꾸므로
이전 준비 예산·feasibility·빈 추천 DB·합성 및 checkpoint 보고서를 다시 만든다. 실패 자원 보고서와
probe 이력은 보존하며 기존 진입점이 실패 보고서를 보관하고 새 예산으로 검사한다. 원본 입력과
공식 checkpoint cache는 유지한다. 배치 32의 L4 통과와 배치 1 대비 점수 일치는 아직 확인 전이다.
