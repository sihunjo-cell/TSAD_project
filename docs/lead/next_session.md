...
2026-09-12 PCA 성분 거리에도 현재 할당 CPU를 적용한다. 기존 행 묶음을 스레드로 나눠
같은 가중 거리식을 계산하고 결과를 원래 행에 돌려놓는다. fit·거리 단계와 묶음 완료 진행을
터미널에 표시하며 실제 작업 수·선택 성분 수를 저장한다. 비용 비교의 관측쌍은 fit·거리의
병렬 수가 모두 맞아야 사용한다. 기존 1스레드 실측 재사용과 설정 간 시간비 추정은 유지한다.

사용자 원격 기록에서 `cb3ca230f385a`(`n_components=0.5`)는 총 44.5분, fit 44.26분,
전처리 0.17분·성분 거리 0.04분에 완료됐다. 현재 `cdfd85c410710`은 `n_components=None`,
attempt 0, 시작 2026-09-11T17:18:01.739818+00:00, 상태 running이다. 완료 시 저장하는
단계 기록이 null인 것만으로 멈춤이나 현재 단계를 판정하지 않는다. 가용 16개 중 약 1.01 CPU
사용을 관측했고 이전 코드는 fit 뒤 거리가 직렬이었다. 전체 성분 후보에는 0.5의 단계별 비율을
적용하지 않는다. series 13의 248채널·window 100은 zero pruning 전 24,800열이다.

현재 게이트는 거리 병렬화 수정·정적 검토, Lightning 회귀 전이다. 로컬 Python·테스트·모델은
실행하지 않는다. 다음은 원격 `TestPcaOfficial`·`test_runtime_comparisons`·
`test_scoring_environment`·`test_resource_resume`와 PCA CPU 시간 보존 회귀다.
실행 중인 checkout에는 pull하지 않는다. 정상 중단 후 새 코드를 받으면 미완료 trial은 분해부터
다시 실행하며 기존 완료 결과와 중단 비용은 보존한다. 현재 trial의 남은 시간과 새 코드의 가속률은
확인 전이다. `d509977`의 회귀 9개 통과(7.947초)와 완료 수 3,835는 충돌 복구의 진행 근거다.

2026-09-12 Lightning 합성 SVD에서 시작/실행 스레드 1/1은 0.99초에 통과했고,
1/16은 SIGSEGV(-11), 16/16은 1.78초에 통과했다. 실제 PCA의 첫 fit → SciPy SVD 충돌과
같은 전환 경로를 재현했다. 진입점에서 NumPy·SciPy의 OpenBLAS를 affinity CPU 수로 먼저
초기화하고 평상시 BLAS·자식 환경을 1스레드로 되돌린다. PCA fit만 기존 가용 CPU 수를 쓰며
비용 비교식·다른 모델·VUS 작업 수 정책은 유지한다. 확인된 예산 `b2f61f74691c6`의 PCA
series 13·`cb3ca230f385a`·r100·seed 0에 한해 attempt 3을 한 번 허용하고 새 이력에 사유를
기록한다. 이전 이력과 예산 파일은 그대로 둔다.

현재 게이트는 수정·정적 검토, 원격 회귀 전이다. 다음은 `test_blas_startup`의 새 프로세스에서
1→가용 CPU→1 복원·PCA 대조·자식 1스레드를 확인하고, 해당 trial의 복구·이력 보존·추가 재시도
차단과 소스 재개 호환 회귀를 실행하는 작업이다. 통과하면 기존 튜닝 명령으로 재개한다.
로컬 Python·테스트·모델은 실행하지 않았고 실제 큰 입력의 성공·가속도 아직 확인하지 않았다.
...
옛 기대값을 경고·통과로 맞췄다. 모델·실행·검사 코드는 추가로 바꾸지 않았다. 다음은 실패했던
두 회귀를 원격에서 확인한 뒤 같은 명령으로 재개하는 작업이다. 이번 로그에서는 && 앞의 테스트가
실패했으므로 튜닝 재개 명령은 실행되지 않았다.

## 직전 수정

2026-09-10 RAM 총량이 8,192바이트 늘어 74% 실행의 재개가 차단된 원인을 수정했다.
사용자 지시에 따라 RAM 80% 초과도 GPU와 같이 경고로 두고 실제 실행 오류는 실패로 남긴다.
저장된 PCA 추정은 당시 RAM 총량으로 검증한다. 이번 검사·재개 연결 수정만 소스 호환으로
인정하며 기존 점수·DB·snapshot의 commit은 보존하고 새 실행에는 실제 새 commit을 기록한다.
모델·후보·배치·학습·점수·선택식은 변경하지 않았다. 현재 게이트는 정적 검토 후 원격 회귀 전이다.
다음은 브랜치를 받은 Lightning에서 관련 회귀를 한 번 확인한 뒤 기존 명령으로 재개하는 작업이다.
보고서나 튜닝 산출물을 삭제·이동할 필요가 없다. 로컬 Python·테스트는 실행하지 않았다.

## 직전 실행 기록

2026-09-09 Lightning은 TSPulse VRAM 96.23% 경고 후 자원 검사를 통과했고, MWVAR96과
Centered-5 앙상블의 상수 성분 점수에서 멈췄다. 공식 단변량 min-max에는 0 범위 처리가 없다.
다변량 확장에서 한 채널의 상수 점수 때문에 전체 실행을 막지 않도록, 정확히 0인 성분 범위만
정규화 결과 0으로 처리한다. 정상 성분·채널·후보·결합식은 보존하고 처리 정책과 채널을 기록한다.
현재 게이트는 이 예외 처리 수정·정적 검토 후 원격 회귀 전이다. 다음은 Lightning에서
`TestOneLinerEnsemble` 4개를 실행하는 작업이다. 본 튜닝 완주는 아직 확인 전이다.

2026-09-09 commit `84952f6`의 Lightning PCA 회귀는 5개 중 4개 통과, 1개 실패다.
실패한 full SVD 비교의 최대 상대 차이는 0.23388895다. 원격 계측에서 실제 입력은 F-order,
테스트의 비교용 복사본은 C-order였다. F-order의 auto·full 최소 가중치는 둘 다
1.3907007850040194e-32이고 C-order의 full은 1.0654312403089098e-32였다.
비교용 복사를 `copy(order="K")`로 고쳐 배열 배치를 유지한다. 모델·후보·점수식·허용 오차는
바꾸지 않았다. 현재 게이트는 테스트 수정·정적 검토 후 원격 재검증 전이다. 다음은 Lightning의
`TestPcaOfficial` 5개 재실행이다. 다른 배열 배치에서도 점수가 안정적이라는 근거로 쓰지 않는다.

2026-09-09 Lightning에서 이전 TSPulse 관련 회귀 4개와 자원 검사를 통과한 뒤 PCA series 11에서
멈췄다. `n_components=None`의 9,401×300 입력은 auto의 covariance_eigh에서 가중치 하나가
0이 됐으며, 원격 단일 파일 대조에서 full SVD는 0 가중치 없이 유한한 점수 9,500개를 만들었다.
이 경우에만 full SVD로 재계산하고 실제 solver를 metadata·checkpoint에 남기도록 수정했다.
후보·전처리·성분 수·가중 거리식은 유지한다. 현재 게이트는 PCA 수정·정적 검토, 원격 회귀 전이다.
로컬 Python·테스트·모델은 실행하지 않았다. 다음은 Lightning의 `TestPcaOfficial` 회귀다.
통과 뒤 새 commit으로 실행하려면 현재 Dev18 점수·manifest·추천 DB·실행 이력과 추천 계약,
합성·자원 보고서를 보관 폴더로 옮기고 기존 명령을 실행한다. 이미 세 번 실패했으므로 빈 DB만
삭제하는 아래 과거 절차를 쓰지 않는다. 예산·feasibility·runtime·Tier 3 checkpoint 보고서와
원본 데이터·모델 cache는 유지한다. 수정한 진입점으로 전체 튜닝을 마친 것은 아직 아니다.
