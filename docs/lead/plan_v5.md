...
2026-09-12 PCA의 가용 CPU 적용 범위를 분해 뒤 성분 거리까지 확장한다. 공식 후보·성분 수·
거리식은 유지하며 독립 행 묶음만 스레드로 병렬 처리한다. 비용 비교의 속도비는 분해와 거리의
병렬 수가 모두 같은 관측쌍에서 구한다. 기존 1스레드 실측과 설정 간 비율 추정은 보존한다.
현재는 수정·정적 검토, 원격 회귀 전이며 실행 중 trial에 새 코드를 끼워 넣지 않는다.
