...
- 고정 GraGOD 포크 `gragod/training/trainer.py:119-138, 206-228`: Adam 뒤에 ReduceLROnPlateau를 붙이고 gradient clipping 1.0을 적용한다. 러너가 그대로 상속하므로 별도 설정을 만들지 않는다.
...
| GHL·HAI `topk` | 5·22 | D-17의 `max(5, ceil(0.25N))`에 `N=19·86`을 대입했다. |
...
| `shuffle` | true | d-ailin train loader와 GraGOD train 설정이 같다. 시계열 순서는 각 window 안에서 보존된다. |
